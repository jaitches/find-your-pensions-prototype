...
/*            let ObjectID = require('mongodb').ObjectID
            let hex_id = req.params.id

            var newObjectId = new ObjectID.createFromHexString(hex_id)
           var Ob_ID = require('mongodb').ObjectID
            var o_id = new Ob_ID.createFromHexString(hex_id)
...
*/           
    ...
    async function printIfListingExists(client, pensionId) {
        const result = await client.db("pensions").collection("pensionDetails")
            .findOne({_id: ObjectId(pensionId)})
            if (result) {
                if (result.last_scraped) {
                    console.log(`Found a listing in the collection with the name '${pensionId}'. Listing was last scraped ${result.last_scraped}.`);
                } else {
                    console.log(`Found a listing in the collection with the name '${pensionId}'`);
                }
            } else {
                console.log(`No listings found with the name '${pensionId}'`);
            }
    }
})
