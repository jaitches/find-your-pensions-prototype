...
const uri = "mongodb+srv://all_dbs_user:U5oZLxA850eM8TFr@cluster0.de9k1.mongodb.net/pensions?retryWrites=true&w=majority";
    ...
    try {
        ...
        await findOneListingByName(client, "Aviva");
        ...
        await findListingsWithMinimumsERI(client, {
            minimumERI: 10
        });
...
async function findOneListingByName(client, nameOfListing) {
    ...
    const result = await client.db("sample_airbnb").collection("listingsAndReviews").findOne({ administratorName: nameOfListing });
 ...
 */
async function findListingsWithMinimumsERI(client) {

    // See https://mongodb.github.io/node-mongodb-native/3.6/api/Collection.html#find for the find() docs
    const cursor = client.db("pensions").collection("lpensionDetails").find()
    ...
    // Print the results
    if (results.length > 0) {
        console.log(`Found listing(s) with at least ${minimumERI} ERI:`);
        ...
        console.log(`No listings found with at least ${minimumERI} ERI`);
    }
}
